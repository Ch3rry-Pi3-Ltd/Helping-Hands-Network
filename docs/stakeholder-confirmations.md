# Stakeholder confirmation pack

For: Susan and the Healing Hands Network trustees

Prepared: 4 July 2026. Updated with Sue's August 2026 feedback.

## Purpose

The replacement website is available as a separate Vercel preview. Before any
live-domain decision, the charity needs to confirm the operational details
below.

Please tick an option or add a short response. Do not put bank details,
passwords, private beneficiary information or other sensitive personal data in
this document or the Git repository. Supply sensitive material through a
separately agreed secure route.

## 1. Primary donation route

Which destination should the main Donate and Support buttons use?

- [ ] Healing Hands Network’s general JustGiving charity profile.
- [ ] The JustGiving “Aid for Ukraine” campaign.
- [x] GoFundMe as the preferred main online fundraising platform.
- [ ] GivingLottery.
- [ ] A different approved route: ______________________________
- [ ] Keep the Support Us hub as the main destination.

Comments:

Sue confirmed on 20 August 2026 that GoFundMe is preferred because it has
lower costs for the charity. The exact preferred GoFundMe URL and current
campaign wording still need final confirmation.

Sue supplied a CAF Donate link on 22 August 2026:
`https://cafdonate.cafonline.org/23120`.

## 2. Public fundraising pages

The following pages were publicly reachable on 4 July 2026. Reachability does
not confirm that the charity still controls, endorses or wants to promote a
page.

For each route, mark **Use**, **Keep but secondary**, or **Do not link**.

### JustGiving charity profile

[Healing Hands Network on JustGiving](https://www.justgiving.com/charity/healinghandsnetwork)

- [ ] Use
- [x] Keep but secondary
- [ ] Do not link
- [ ] The public description and charity details are approved.
- [ ] The charity has current administrator access.

### JustGiving — Aid for Ukraine

[Aid for Ukraine](https://www.justgiving.com/campaign/justgivingaidforukraine)

- [ ] Use
- [ ] Keep but secondary
- [ ] Do not link
- [ ] The campaign remains open for donations.
- [ ] The campaign wording, named partners and activities remain current.

### GoFundMe — Aid for Ukraine continues

[Aid for Ukraine continues](https://www.gofundme.com/f/aid-for-ukraine-continues)

- [x] Use as preferred main online platform, subject to exact URL confirmation
- [ ] Keep but secondary
- [ ] Do not link
- [ ] The campaign remains open for donations.
- [ ] The charity has current administrator access.
- [ ] The campaign wording and spending purpose remain current.
- [ ] The exact current GoFundMe URL has been supplied. The latest pasted
      GoFundMe-style link did not resolve as a campaign when checked on
      22 August 2026.

### CAF Donate

[CAF Donate](https://cafdonate.cafonline.org/23120)

- [x] Sue supplied this link on 22 August 2026.
- [x] The page returned HTTP 200 when checked.
- [ ] Confirm desired prominence and approved wording.

### GoFundMe — Kherson animal shelter

[Help for Ukraine — Kherson Irina dog and cat shelter](https://www.gofundme.com/f/help-for-ukraine-kherson-irina-dog-and-cat-shelter)

- [ ] Use
- [ ] Keep but secondary
- [ ] Do not link
- [ ] The campaign remains open for donations.
- [ ] The campaign need and intended delivery remain current.

### GivingLottery

[Healing Hands Network on GivingLottery](https://www.givinglottery.org.uk/superdraw/healing-hands-network)

- [x] Use, subject to current link and required wording
- [ ] Keep but secondary
- [ ] Do not link
- [ ] The charity currently participates in and approves this route.
- [ ] The charity has current administrator access.
- [ ] Any required lottery wording or age restriction has been reviewed.

### Give as You Live

- [x] Sue confirmed the charity uses Give as You Live.
- [ ] Current public link supplied.
- [ ] Approved promotional wording or marketing material supplied.

## 3. Forms and offline support

Please supply or identify the current approved version of each item:

- [ ] Standing-order form.
- [ ] Gift Aid declaration.
- [ ] Membership application.
- [ ] Sponsorship form.
- [ ] General donation form.
- [ ] Collecting-tin guidance or form.
- [ ] Instructions for supporters who need an accessible/offline option.

Who owns future updates to these documents?

______________________________________________________________________

## 4. Public contact information

Confirm what may be published:

- [x] Email: `healinghandsnetwork@gmail.com`
- [x] Admin support email: `emmahhn1@gmail.com`
- [x] Telephone: `07734 462000`
- [ ] Address:
      `Healing Hands Network, 151 Fillongley Road, Meriden, Coventry, CV7 7LT`
- [ ] Facebook: `facebook.com/healinghandsnetwork`

Preferred contact route for:

- General enquiries: _______________________________________________
- Volunteering: ____________________________________________________
- Donations and Gift Aid: __________________________________________
- Admin support: `emmahhn1@gmail.com`
- Safeguarding or sensitive enquiries: ______________________________

## 5. Current programme information

### Bosnia and Herzegovina

- [x] Current season confirmed as usually 1 May to 31 July, extendable if
      enough volunteers are available.
- [ ] Current outreach locations.
- [x] Partner-association categories supplied by Sue.
- [ ] Exact partner names, spelling and permission to publish confirmed.
- [x] Volunteer cost coverage supplied by Sue.
- [ ] Current volunteer contribution amount, if any, confirmed.
- [ ] Approved description of therapies currently offered.

### Ukraine

- [ ] Current named partners.
- [ ] Current delivery activity.
- [ ] Current campaign priorities and requested items.
- [ ] Approved wording about vehicles, medical and humanitarian aid.
- [x] Approximate delivery-trip cost supplied as £1,600.
- [ ] Confirm whether the approximate £1,600 trip cost should be used in
      prominent fundraising copy.
- [ ] Amazon wishlist reviewed for current items, availability and prices.
- [ ] Any partners or activities that should no longer be named.

### Current UK support

- [x] Current UK work confirmed as FABCAMPS support in Wales and Scotland.
- [x] Wider UK veterans work confirmed as limited/not active since Covid.
- [ ] Approved FABCAMPS wording.
- [ ] Confirm whether this page should remain separate or be folded into Our
      Work.

## 6. Volunteering and membership

- [ ] Current membership fees.
- [x] Therapist volunteers/supporter members should be professionally
      qualified and insured in mainstream modalities.
- [x] Non-therapists may join as supporters.
- [ ] Detailed eligibility criteria for therapist volunteers.
- [ ] Required checks.
- [ ] Current induction or trauma-aware training requirements.
- [ ] Current opportunities for non-therapist supporters.
- [ ] The person who receives and manages enquiries.
- [ ] Approved application and privacy process.

## 7. News, impact and evidence

- [ ] Supply the first approved news or project updates.
- [ ] Supply publication dates and a project label for each update.
- [ ] Identify which impact figures may be published.
- [ ] Identify the record or report supporting each figure.
- [ ] Approve any testimonials and attribution wording.
- [ ] Confirm whether any quotation should be anonymous.

## 8. Images, consent and safeguarding

For every logo and photograph used in the preview:

- [ ] The charity owns it or has permission to publish it.
- [ ] People shown have appropriate consent.
- [ ] Publication does not create a safeguarding or security risk.
- [ ] The proposed caption and alternative text are accurate and dignified.
- [ ] Any images that must be removed are listed below.

Images to remove or replace:

______________________________________________________________________

Sue has offered to send photos and videos. For each new asset, confirm:

- [ ] ownership/permission;
- [ ] consent for identifiable people;
- [ ] caption;
- [ ] alternative text;
- [ ] whether locations, names or sensitive details should be avoided;
- [ ] video hosting/transcript/subtitle approach where relevant.

## 9. Website approval and maintenance

- [ ] The overall design remains recognisably Healing Hands Network.
- [ ] The navigation and page structure are approved.
- [ ] The calls to Donate, Volunteer and Contact are appropriate.
- [ ] The Vercel preview may be shared with named reviewers.
- [ ] A final production domain has been agreed.
- [ ] A person is nominated to approve future website updates.
- [ ] A person is nominated to maintain fundraising links and forms.

Named reviewers and roles:

______________________________________________________________________

Target review date:

______________________________________________________________________
